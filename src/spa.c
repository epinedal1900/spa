#include <math.h>
#include "spa.h" // Incluye el archivo de cabecera SPA para prototipos de funciones y constantes
...
// Truncar un double a un entero
int integer(double value)
{
    return value;
}

// Normalizar un ángulo en grados al rango [0, 360)
double limit_degrees(double degrees)
{
    double limited;

    degrees /= 360.0;                             // Divide 'degrees' entre 360.0 para obtener el número de vueltas completas
    limited = 360.0 * (degrees - floor(degrees)); // Resta el número entero de vueltas (usando floor) y multiplica el resultado por 360 para obtener el ángulo en el rango de 0 a 360 grados

    // Si el resultado es negativo, le suma 360 para asegurarse de que esté en el rango de 0 a 360 grados
    if (limited < 0)
        limited += 360.0;
...
double limit_degrees180pm(double degrees)
{
    ...
    // Si el resultado es menor que -180 grados, le suma 360 para llevarlo al rango de -180 a 180 grados
    if (limited < -180.0)
        limited += 360.0;
    // Si el resultado es mayor que 180 grados, le resta 360 para llevarlo al rango de -180 a 180 grados
...
double limit_degrees180(double degrees)
{
    ...
    degrees /= 180.0;                             // Divide 'degrees' entre 180.0 para obtener el número de medias vueltas
    limited = 180.0 * (degrees - floor(degrees)); // Resta el número entero de medias vueltas (usando floor) y multiplica el resultado por 180 para obtener el ángulo en el rango de 0 a 180 grados

    // Si el resultado es negativo, le suma 180 para asegurarse de que esté en el rango de 0 a 180 grados
    if (limited < 0)
        limited += 180.0;
    return limited;
}
...
double limit_zero2one(double value)
{
    ...
    limited = value - floor(value); // Resta el mayor entero menor o igual a 'value' (usando floor), dejando la parte fraccional

    // Si el resultado es negativo, le suma 1.0 para asegurarse de que esté en el rango de 0 (inclusive) a 1 (exclusivo)
    if (limited < 0)
        limited += 1.0;
...
// Normalizar minutos al rango [-20, 20) o equivalente
double limit_minutes(double minutes)
{
    double limited = minutes;

    // Si 'limited' es menor que -20.0, suma 1440.0 para ajustarlo
    // Para manejar valores que representan minutos negativos en un contexto de 24 horas
    if (limited < -20.0)
        limited += 1440.0;

    // Si 'limited' es mayor que 20.0, resta 1440.0 para ajustarlo
    // Para manejar valores que representan minutos que exceden un umbral en un contexto de 24 horas
...
// Convertir una fracción del día a horas locales, considerando la zona horaria
double dayfrac_to_local_hr(double dayfrac, double timezone)
{
    // Convierte la fracción del día a horas, teniendo en cuenta la zona horaria
    // dayfrac representa la fracción del día, y timezone es el desplazamiento de la zona horaria en horas
    return 24.0 * limit_zero2one(dayfrac + timezone / 24.0);
}

// Calcular el valor de un polinomio de tercer orden
...
///////////////////////////////////////////////////////////////////////////////////////////////
// Validar las entradas para el algoritmo SPA
...
double julian_day(int year, int month, int day, int hour, int minute, double second, double dut1, double tz)
{
    ...
    // Convierte la fecha y hora local a un valor decimal que representa el día
    day_decimal = day + (hour - tz + (minute + (second + dut1) / 60.0) / 60.0) / 24.0;

    // Ajusta el mes y el año si el mes es enero o febrero
    if (month < 3)
    {
        month += 12;
        year--;
    ...
    // Calcula la primera parte del día juliano
    julian_day = integer(365.25 * (year + 4716.0)) + integer(30.6001 * (month + 1)) + day_decimal - 1524.5;

    // Ajusta el día juliano para fechas después de la introducción del calendario gregoriano en octubre de 1582
...
// Calcula el siglo juliano desde el día juliano (jd)
double julian_century(double jd)
{
    // 2451545.0 es el día juliano al inicio del siglo J2000 (1 de enero de 2000, al mediodía)
    // 36525.0 es el número de días en un siglo juliano
    return (jd - 2451545.0) / 36525.0;
}

// Calcula el día efemérides juliano, que incluye la corrección del tiempo ΔT (delta_t)
double julian_ephemeris_day(double jd, double delta_t)
{
    // 86400.0 es el número de segundos en un día
    // Se añade delta_t (en segundos) al día juliano (jd)
    return jd + delta_t / 86400.0;
}

// Calcula el siglo efemérides juliano desde el día efemérides juliano (jde)
double julian_ephemeris_century(double jde)
{
    // Similar a julian_century, pero usando el día efemérides juliano (jde)
    return (jde - 2451545.0) / 36525.0;
}

// Calcula el milenio efemérides juliano desde el siglo efemérides juliano (jce)
double julian_ephemeris_millennium(double jce)
{
    // Divide el siglo efemérides juliano (jce) por 10 para obtener el milenio
    return (jce / 10.0);
}
...
double geocentric_right_ascension(double lamda, double epsilon, double beta)
{
    ...
    return limit_degrees(rad2deg(atan2(sin(lamda_rad) * cos(epsilon_rad) -
                                           tan(deg2rad(beta)) * sin(epsilon_rad),
                                       cos(lamda_rad))));
}
